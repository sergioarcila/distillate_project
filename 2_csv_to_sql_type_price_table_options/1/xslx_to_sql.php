...
$qry = "CREATE TABLE IF NOT EXISTS `".$DB_tableName1."` ( id INT NOT NULL AUTO_INCREMENT, `TradeDate` date NOT NULL, `Contractdate` date NOT NULL, `PriceCode` int NOT NULL, `OptionTypeCode` float NOT NULL, `StrikePrice` float NOT NULL, `SettlementPrice` float NOT NULL, `HighPrice` float NOT NULL, `LowPrice` float NOT NULL, `Volume` float NOT NULL, `Previous` float NOT NULL, PRIMARY KEY (id) ) ENGINE = MYISAM DEFAULT CHARSET=utf8 ;";
$db->select($qry);
    ...
    for($j=1; $j<$count_line2; $j++){
        ...
        if(!empty($result_AUS[$j][0]) and !empty($result_AUS[$j][1])){
            ...
            if(!empty($result_AUS[$j][2])){
                $PriceCode = trim(preg_replace("/,/",".",trim($result_AUS[$j][2])));
            }
            if(!empty($result_AUS[$j][3])){
                $OptionTypeCode = trim(preg_replace("/,/",".",trim($result_AUS[$j][3])));
            }
            if(!empty($result_AUS[$j][4])){
                $StrikePrice = trim(preg_replace("/,/",".",trim($result_AUS[$j][4])));
            }
            if(!empty($result_AUS[$j][5])){
                $SettlementPrice = trim(preg_replace("/,/",".",trim($result_AUS[$j][5])));
            }
            if(!empty($result_AUS[$j][6])){
                $HighPrice = trim(preg_replace("/,/",".",trim($result_AUS[$j][6])));
            }
            if(!empty($result_AUS[$j][7])){
                $LowPrice = trim(preg_replace("/,/",".",trim($result_AUS[$j][7])));
            }
            if(!empty($result_AUS[$j][8])){
                $Volume = trim(preg_replace("/,/",".",trim($result_AUS[$j][8])));
            }
            if(!empty($result_AUS[$j][9])){
                $Previous = trim(preg_replace("/,/",".",trim($result_AUS[$j][9])));
            }

            $qry = "INSERT INTO `".$DB_tableName1."` (`id`, `TradeDate`, `Contractdate`, `PriceCode`, `OptionTypeCode`, `StrikePrice`, `SettlementPrice`, `HighPrice`, `LowPrice`, `Volume`, `Previous`) VALUES (NULL, '".$TradeDate."', '".$Contractdate."', '".$PriceCode."', '".$OptionTypeCode."', '".$StrikePrice."', '".$SettlementPrice."', '".$HighPrice."', '".$LowPrice."', '".$Volume."', '".$Previous."');";
            $db->select($qry);
